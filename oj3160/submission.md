...
```text
Input:
รับค่า a และ b ในบรรทัดเดียว

Output:
แสดงจำนวนเฉพาะทั้งหมดในช่วงตั้งแต่ a ถึง b 
แสดงจำนวนจำนวนเฉพาะทั้งหมดในช่วงตั้งแต่ a ถึง b ในรูปแบบ Total primes: 

Constraints:
หากไม่มีจำนวนเฉพาะในช่วงดังกล่าว ให้แสดงเฉพาะ Total primes: 0
```
...
```text
Step 1: รับค่า a b ในบรรทัดเดียว
Step 2: สร้างลิสต์ว่างสำหรับเก็บจำนวนเฉพาะที่พบ
Step 3: วนลูปตรวจสอบตัวเลขตั้งแต่ a ถึง b
Step 4: ตรวจสอบว่าตัวเลขนั้นเป็นจำนวนเฉพาะหรือไม่ (มากกว่า 1 และไม่มีตัวใดนอกจาก 1 กับตัวมันเองหารลงตัว) ถ้าเป็นจำนวนเฉพาะให้เพิ่มเข้าลิสต์
Step 5: แสดงผล
```
...
```text
ลูปเช็คเงื่อนไขจำนวนเฉพาะ โดยถ้าน้อยกว่าหรือเทา่กับ 1 หรือ เป็นเลขคู่ที่ไม่ใช่ 2 จะไม่เป็นจำนวนเฉพาะ เลขอื่นๆ ให้ใช้ลูปวนหารตั้งแต่ 3 ถึง ตัวมันเอง ถ้าไม่มีตัวหารลงตัวจะเป็นจำนวนเฉพาะ
```
