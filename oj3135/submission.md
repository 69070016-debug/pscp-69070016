...
```text
Input:
รับค่า 3 จำนวนในบรรทัดเดียว n k t

Output:
แสดงจำนวนคนที่ได้พิจารณาของขวัญ

Constraints:
เริ่มจากคนที่ 1 และส่งต่อทีละ k คน ถ้าวนกลับมาที่คนที่ 1 หรือของขวัญถึงมือขโมย ให้หยุด
```
...
```text
Step 1: รับค่า n k t
Step 2: ใช้ลูป while วนส่งของขวัญต่อไปทีละ k คน
Step 3: ถ้าตำแหน่งตรงกับขโมย หรือวนกลับมาที่คนที่ 1 ให้หยุดลูป
Step 4:แสดงผลจำนวนคนที่ได้พิจารณาของขวัญออกทางหน้าจอ
```
...
```text
เหมือนแผนแรก ใช้ลูป while cycle != t: บวกตำแหน่งเพิ่มทีละ k หากตำแหน่งเกิน n ให้ปรับด้วยการมอดุโล % n ถ้าได้ 0 ให้เป็น n และเพิ่มตัวนับ r += 1 หากวนกลับมาที่คนที่ 1 ให้หักตัวนับออก 1 แล้วสิ้นสุดการทำงาน
```
...
```text
เพื่อดูว่าถ้าของขวัญอยู่ในมือขโมยจะต้องหยุดนับ
```
